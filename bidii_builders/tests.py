...
class EstimateModelTest(TestCase):
    ...
    def test_estimate_creation(self):
        ...
        self.assertEqual(
            str(estimate), f"Estimate #{estimate.id} - {self.customer.full_name}"
        )
...
class APIViewTest(TestCase):
    ...
    def test_create_property_api(self):
        ...
        response = self.client.post(
            "/api/create-property/", content_type="application/json", data=post_data
        )
        self.assertEqual(response.status_code, 200)
...
class BusinessLogicTest(TestCase):
    ...
    def test_invoice_creation_from_job(self):
        ...
        invoice = Invoice.objects.create(
            job=job, amount=Decimal("48000.00"), due_date=date.today()
        )
